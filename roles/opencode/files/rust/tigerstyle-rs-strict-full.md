# TigerStyle Rulebook — Rust / Strict / Full

## Preamble

### Purpose

This document is a comprehensive Rust coding rulebook derived from TigerBeetle's TigerStyle. It is
intended to be dropped into any Rust codebase as part of an `AGENTS.md` file, a system prompt, or a
code review checklist. Every rule is actionable and enforceable.

This is the **Rust-specific** variant. Rule IDs match the language-agnostic TigerStyle rulebook for
cross-referencing. Where Rust idioms differ from the language-agnostic version, the adaptation is
noted.
...
### Rust Baseline and Tooling

- Target: **latest stable Rust**.
- Formatter: `rustfmt` (mandatory).
- Lint: `clippy` with `-D warnings` (mandatory).
- Tests: `cargo test` must pass; use `--all-features` when applicable.
- Warnings are errors: `#![deny(warnings)]` in crate root.

### Unsafe Policy

- `unsafe` MUST be minimized.
- Every `unsafe` block MUST include a comment explaining the safety invariants.
- The smallest possible scope MUST be used for `unsafe` blocks.

### Error Handling Policy (Rust)

- `unwrap`/`expect` MUST NOT be used outside tests unless:
  1) the invariant is asserted immediately nearby, and
  2) the rationale is documented in a comment.
- Prefer `expect` with a clear message when allowed.
...
- Each rule has: an imperative statement, a rationale, and a Rust example or template.
- Rules marked **(Rs-adapted)** have been adjusted from the language-agnostic version.
...
for attempt in 0..MAX_RETRIES {
    ...
    assert!(attempt < MAX_RETRIES - 1, "connection retries exhausted");
}
```

### SAF-03 — Use explicitly-sized integer types. **(Rs-adapted)**
...
// Avoid: usize unless indexing or stdlib requires it
let index: usize = items.len(); // allowed only for indexing
```
...
```rust
fn process_batch(items: &[Item], max_size: usize) -> Vec<Result> {
    assert!(items.len() <= max_size, "batch exceeds max size");
    let results = do_work(items);
    assert!(results.len() == items.len(), "result count mismatch");
    results
}
```
...
let loaded = read_from_disk(record.id)?;
...
const _: () = assert!(BLOCK_SIZE % PAGE_SIZE == 0);
```
...
```rust
if index < length {
    assert!(buffer[index].is_some(), "slot must be populated");
} else {
...
### SAF-12 — Pre-allocate and reuse. Avoid unbounded allocations. **(Rs-adapted)**

Allocations in hot paths MUST be minimized. Collections MUST be pre-allocated where the upper bound
is known. Avoid per-event allocations when reuse is feasible.

Rationale: Excessive allocation increases allocator overhead and tail latency. Pre-allocation and
reuse improve predictability.
...
### SAF-17 — Treat all warnings as errors; require rustfmt and clippy. **(Rs-adapted)**

`rustfmt` MUST be run. `clippy -D warnings` MUST pass. `#![deny(warnings)]` MUST be enabled.
...
Compound boolean conditions MUST be split into nested if/else branches. Complex `else if` chains
MUST be rewritten as nested `else { if { } }` trees.
...
### SAF-21 — Handle all errors explicitly. Avoid `unwrap`/`expect` unless justified. **(Rs-adapted)**

Every error MUST be handled explicitly. `unwrap`/`expect` MUST NOT be used outside tests unless the
invariant is asserted nearby and the rationale is documented. `Result` and `Option` MUST be handled.

Rationale: Error-handling bugs are the dominant cause of catastrophic production failures.

```rust
let value = map.get(key).expect("key must exist: validated by caller");
// Why: key presence is asserted in the caller before this call.
```
...
### PERF-08 — Use primitive arguments in hot loops. Avoid large receiver access. **(Rs-adapted)**

Hot loop functions MUST take primitive arguments directly. They MUST NOT access large `self` fields
in tight loops when performance matters.

Rationale: Primitive arguments allow the compiler to keep values in registers and avoid alias
analysis overhead.

```rust
fn process_range(data: &[f64], start: usize, end: usize) -> f64 {
    let mut sum = 0.0;
    for i in start..end {
        sum += data[i];
    }
...
### DX-02 — Use Rust naming conventions. **(Rs-adapted)**

Functions, variables, and modules MUST use `snake_case`. Types, traits, and enums MUST use
`PascalCase`. Constants MUST use `SCREAMING_SNAKE_CASE`. File names MUST use `snake_case`.
...
fn transfer(options: TransferOptions) {
    // ...
}
```
...
### CIS-02 — Pass large arguments by reference. **(Rs-adapted)**

Function arguments larger than 16 bytes MUST be passed by reference (`&T`), not by value.

Rationale: Passing large structs by value creates implicit copies that waste stack space.

### CIS-03 — Prefer in-place initialization via builders or `MaybeUninit`. **(Rs-adapted)**

Large structs MUST be initialized in-place (via builders or `MaybeUninit`) in performance-critical
code, rather than returned by value when copies matter.

Rationale: In-place initialization avoids intermediate copies and ensures pointer stability.

### CIS-04 — If any field requires in-place init, the whole struct does. **(Rs-adapted)**
...
Return types MUST be as simple as possible: `()` > `bool` > integer > `Option<T>` > `Result<T, E>`.

Rationale: Each additional dimension creates branches at every call site.

### CIS-07 — Do not `await` between assertions and dependent code. **(Rs-adapted)**
...
All integer division MUST use explicit semantics: exact, floor, or ceiling. Use helper functions to
...
All code MUST be formatted by `rustfmt`. No manual formatting overrides are permitted.

Rationale: Automated formatting eliminates style debates and ensures consistency.

### FMT-02 — Use 4-space indentation.

Indentation MUST be 4 spaces (as produced by `rustfmt`).
...
### DEP-03 — Prefer Rust for scripts and automation. **(Rs-adapted)**

Scripts and automation MUST be written in Rust. Shell scripts are acceptable only for trivial glue
