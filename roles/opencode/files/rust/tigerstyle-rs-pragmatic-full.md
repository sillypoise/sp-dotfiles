# TigerStyle Rulebook — Rust / Pragmatic / Full
...
- Tests: `cargo test` should pass; use `--all-features` when applicable.
- Prefer `#![deny(warnings)]`.

### Unsafe Policy

- `unsafe` SHOULD be minimized.
- Every `unsafe` block SHOULD include a comment explaining the safety invariants.
- Prefer the smallest possible scope for `unsafe` blocks.

### Error Handling Policy (Rust)

- `unwrap`/`expect` SHOULD be avoided outside tests.
- If used, the invariant MUST be documented and an `expect` message MUST explain why it is safe.
...
- Each rule has: a recommendation, a rationale, and a Rust example or template.
...
Integer types SHOULD be explicitly sized (`u32`, `i64`). AVOID `usize` unless required for indexing
...
Constants and type relationships SHOULD be asserted at compile time (or startup).
...
Allocations in hot paths SHOULD be minimized. Pre-allocate collections when possible.

Rationale: Excessive allocation increases allocator overhead and tail latency.
...
### SAF-17 — Treat warnings as errors; prefer rustfmt and clippy. **(Rs-adapted)**

`rustfmt` and `clippy -D warnings` SHOULD pass with no warnings. Prefer `#![deny(warnings)]`.
...
Every error SHOULD be handled explicitly. AVOID `unwrap`/`expect` outside tests unless the
invariant is documented and an `expect` message explains why it is safe.
...
Hot loop functions SHOULD take primitive arguments directly. AVOID accessing large `self` fields in
tight loops.

Rationale: Primitive arguments are register-friendly and avoid alias analysis overhead.
...
Functions/variables/modules SHOULD use `snake_case`. Types/traits/enums SHOULD use `PascalCase`.
Constants SHOULD use `SCREAMING_SNAKE_CASE`. File names SHOULD use `snake_case`.

Rationale: Rust conventions reduce friction and improve tooling compatibility.
...
Struct definitions SHOULD be ordered: data fields first, then nested types, then methods.
...
Functions with confusable arguments SHOULD use named options structs.
...
Arguments larger than 16 bytes SHOULD be passed by reference.
...
Large structs SHOULD be initialized in-place in performance-critical code.
...
PREFER simpler return types: `()` > `bool` > integer > `Option<T>` > `Result<T, E>`.
...
All code SHOULD be formatted by `rustfmt`.
...
Scripts SHOULD be written in Rust. Shell scripts only for trivial glue (< 20 lines).
