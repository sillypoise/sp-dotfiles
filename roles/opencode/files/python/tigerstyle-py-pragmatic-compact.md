# TigerStyle Rulebook — Python / Pragmatic / Compact

## Preamble

This is the compact Python variant of the TigerStyle Pragmatic rulebook. Each rule is a
recommendation with a one-line rationale. For full rationale and Python examples, see
`tigerstyle-py-pragmatic-full.md`.
...
**SAF-03** **(Py)** — All values SHOULD have precise types. AVOID `Any`. Use `NewType`/`Annotated`, explicit bounds, avoid `float` for counts.
...
**SAF-09** — Constants and type relationships SHOULD be asserted at import time or startup.
...
**SAF-12** **(Py)** — Allocations in hot paths SHOULD be minimized. Pre-size lists/buffers and reuse.
...
**SAF-17** **(Py)** — `black`, `ruff`, and `mypy --strict` SHOULD pass. Tests SHOULD run with warnings as errors.
...
**SAF-21** — Every error SHOULD be handled explicitly. AVOID silent exception swallowing. No bare `except`.
...
**PERF-07** — Performance-critical code SHOULD be explicit. AVOID depending on interpreter optimizations.
Rationale: Interpreter heuristics are fragile and non-portable.

**PERF-08** **(Py)** — Hot loop functions SHOULD take primitive arguments. Avoid repeated global lookups.
...
**DX-11** — Class layout SHOULD follow: fields → types → methods.
...
**DX-14** — Functions with confusable arguments SHOULD use named option objects.
...
**CIS-02** **(Py)** — AVOID shallow-copying large objects via `dict(...)`, `list(...)`, or `copy()` in hot paths.
Rationale: Copying large objects creates GC pressure and can mask mutation bugs.

**CIS-03** **(Py)** — Large objects SHOULD be constructed in-place. AVOID intermediate copies.
Rationale: Intermediate copies waste memory and CPU cycles.

**CIS-04** **(Py)** — If any field requires builder-pattern init, the entire object SHOULD use the same strategy.
...
**CIS-06** — PREFER simpler return types: None > bool > int > T | None > Result-like.
Rationale: Each dimension in the return type creates viral call-site branching.

**CIS-07** **(Py)** — AVOID `await` between an assertion and the code that depends on it. Re-assert after resumption.
...
**FMT-01** — All code SHOULD be formatted by `black` (or `ruff format`) with line length 100.
Rationale: Eliminates style debates and ensures consistency.

**FMT-02** **(Py)** — Indentation SHOULD be 4 spaces. No tabs.
...
**DEP-03** **(Py)** — Scripts SHOULD be written in Python. Shell scripts only for trivial glue (<20 lines).
