# TigerStyle Rulebook — Swift / Pragmatic / Compact

## Preamble

This is the compact Swift variant of the TigerStyle Pragmatic rulebook. Each rule is a
recommendation with a one-line rationale. For full rationale and Swift examples, see
`tigerstyle-swift-pragmatic-full.md`.
...
**Swift baseline:** Swift 5.9. `swift-format` recommended. Treat warnings as errors.
Prefer `swift test` in CI.
...
**SAF-03** **(Sw)** — Integer types SHOULD be explicitly sized (Int64, UInt32).
AVOID Int/UInt unless indexing/API requires.
...
**SAF-07** — PREFER split assertions: `precondition(a); precondition(b);` over
`precondition(a && b)`.
Rationale: Split assertions isolate failure causes and improve readability.

**SAF-08** — PREFER single-line implication asserts: `if a { precondition(b) }`.
...
**SAF-10** — Assertions SHOULD cover both positive space (expected) and negative space
(not expected).
...
**SAF-12** **(Sw)** — Allocations in hot paths SHOULD be minimized. Pre-allocate collections and
reuse.
...
**SAF-17** **(Sw)** — `swift-format` SHOULD pass. Treat warnings as errors
(`-warnings-as-errors`).
...
**SAF-21** **(Sw)** — Every error SHOULD be handled explicitly. AVOID `try?`/`try!`
unless justified.
...
**PERF-04** — Control plane (scheduling, metadata) SHOULD be separated from data plane
(bulk processing).
...
**PERF-08** **(Sw)** — Hot loop functions SHOULD take primitive arguments. AVOID large `self`
...
**CIS-02** **(Sw)** — Arguments larger than 16 bytes SHOULD be passed by reference
(`inout`/reference types).
Rationale: Avoids implicit copies and stack waste.

**CIS-03** **(Sw)** — Large structs SHOULD be initialized in-place via `inout` builders or
pointers.
Rationale: Avoids copies and ensures pointer stability.

**CIS-04** **(Sw)** — If any field requires in-place init, the entire struct SHOULD be
...
**CIS-07** **(Sw)** — AVOID `await` between an assertion and the code that depends on it.
Re-assert after resumption.
...
**OBO-01** — Index, count, and size SHOULD be treated as distinct concepts with explicit
conversions.
...
**FMT-01** — All code SHOULD be formatted by `swift-format`.
...
**DEP-03** **(Sw)** — Scripts SHOULD be written in Swift. Shell scripts only for trivial
glue (<20 lines).
