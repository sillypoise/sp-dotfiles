# TigerStyle Rulebook — Swift / Pragmatic / Full
...
- Warnings SHOULD be treated as errors (`-warnings-as-errors`).
- Tests SHOULD pass with `swift test` (or the project test runner).
...
- Each rule has: a recommendation, a rationale, and a Swift example or template.
...
All control flow SHOULD be simple, explicit, and statically analyzable. Recursion SHOULD be
avoided.
...
Integer types SHOULD be explicitly sized (`Int64`, `UInt32`). AVOID `Int`/`UInt` unless required for
indexing or API compatibility.
...
PREFER expressing implications as: `if condition { precondition(invariant) }`.
...
Constants and type relationships SHOULD be asserted at compile time (or at startup when the
language lacks compile-time assertions).
...
Allocations in hot paths SHOULD be minimized. Pre-allocate collections (e.g., `reserveCapacity`) and
reuse buffers when feasible.
...
`swift-format` SHOULD pass and compiler warnings SHOULD be treated as errors
(`-warnings-as-errors`).
...
Every error SHOULD be handled explicitly. AVOID `try?`/`try!` outside tests unless the invariant is
documented.
...
Functions/variables SHOULD use lowerCamelCase. Types SHOULD use UpperCamelCase. Enum cases SHOULD
use lowerCamelCase. Follow project file naming conventions.
...
Arguments larger than 16 bytes SHOULD be passed by reference (`inout`, reference types, or
...
Large structs SHOULD be initialized in-place in performance-critical code, using `inout` builders or
`UnsafeMutablePointer` where appropriate.
...
PREFER simpler return types: `Void` > `Bool` > integer > optional > `Result`.
...
All code SHOULD be formatted by `swift-format`.
...
Rationale: 4 spaces is a clear visual indentation depth.
...
Scripts SHOULD be written in Swift (or another typed, portable language). Shell scripts only for
trivial glue (< 20 lines).

Rationale: Typed scripts are portable, consistent, and safer than shell.
