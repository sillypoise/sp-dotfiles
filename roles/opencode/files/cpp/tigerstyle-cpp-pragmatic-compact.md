# TigerStyle Rulebook — C++ / Pragmatic / Compact

## Preamble

This is the compact C++ variant of the TigerStyle Pragmatic rulebook. Each rule is a recommendation
with a one-line rationale. For full rationale and C++ examples, see
`tigerstyle-cpp-pragmatic-full.md`.
...
**C++ baseline:** C++20. `clang-format` recommended. `clang`/`gcc` warnings as errors. Sanitizers in
CI.

Rule IDs are stable across all TigerStyle variants for cross-referencing.
Rules marked **(C++)** have been adapted from the language-agnostic version.
...
**SAF-03** **(C++)** — Integer types SHOULD be explicitly sized (`std::int32_t`, `std::int64_t`).
AVOID `size_t` except for indexing/API needs.
...
**SAF-09** **(C++)** — Compile-time constants and type sizes SHOULD be asserted at compile time
(`static_assert`) or startup.
Rationale: Catches design integrity violations before runtime.

**SAF-10** — Assertions SHOULD cover both positive space (expected) and negative space (not
expected).
...
**SAF-13** — Variables SHOULD be declared at the smallest possible scope; minimize variables in
scope.
...
**SAF-16** — State mutation SHOULD be centralized in parent functions. Leaf functions SHOULD be
pure.
Rationale: Localizes bugs to one mutation site; enables testable helpers.

**SAF-17** **(C++)** — All compiler warnings SHOULD be enabled at strictest settings; sanitizers
SHOULD run in CI.
...
**PERF-02** — Back-of-the-envelope calculations SHOULD be performed for network, disk, memory, and
CPU.
...
**PERF-04** — Control plane (scheduling, metadata) SHOULD be separated from data plane (bulk
processing).
...
**PERF-07** — Performance-critical code SHOULD be explicit. AVOID depending on compiler
optimizations.
Rationale: Compiler heuristics are fragile and non-portable.

**PERF-08** — Hot loop functions SHOULD take primitive arguments. AVOID passing large structs by
pointer.
...
**DX-02** **(C++)** — File, function, and variable names SHOULD use snake_case; types follow
project convention.
...
**DX-14** — Functions with confusable arguments (same type, swappable) SHOULD use named option
structs.
...
Rationale: `foo(nullptr)` is meaningless; `foo(timeout_ms: nullptr)` is not.
...
**CIS-01** — Every piece of state SHOULD have exactly one source of truth. AVOID duplication or
aliasing.
Rationale: Duplicated state will desynchronize.

**CIS-02** **(C++)** — Function arguments larger than 16 bytes SHOULD be passed by const reference.
Rationale: Avoids implicit copies and stack waste.

**CIS-03** **(C++)** — Large structs SHOULD be initialized in-place via out pointers.
Rationale: Avoids copies and ensures pointer stability.

**CIS-04** **(C++)** — If any field requires in-place init, the entire struct SHOULD be initialized
in-place.
...
**CIS-06** — PREFER simpler return types: void > bool > int > optional > error type.
Rationale: Each dimension in the return type creates viral call-site branching.

**CIS-07** — Functions with precondition assertions SHOULD run to completion without re-entrancy.
...
**OBO-02** **(C++)** — All integer division SHOULD use explicit semantics: div_exact, div_floor,
div_ceil.
...
**FMT-01** **(C++)** — All code SHOULD be formatted by `clang-format`.
