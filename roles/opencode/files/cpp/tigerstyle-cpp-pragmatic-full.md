...
- Prefer `clang-tidy` for high-signal diagnostics when available.

### Undefined Behavior Policy

- Undefined behavior SHOULD be treated as a correctness defect.
- Type-punning SHOULD use `std::memcpy` or `std::bit_cast`, not aliasing casts.
...
Integer types SHOULD be explicitly sized (`std::int32_t`, `std::int64_t`). AVOID `size_t` and
`long` unless required for indexing or API compatibility.
...
Constants and type relationships SHOULD be asserted at compile time (`static_assert`) or startup.
...
File, function, and variable names SHOULD use `snake_case`. Types SHOULD use consistent
`UpperCamelCase` or project convention.
...
Nullable parameters SHOULD be named so `nullptr` meaning is clear.
...
PREFER simpler return types: void > bool > integer > optional > error type.
