# TigerStyle Rulebook — Go / Strict / Compact

## Preamble

This is the compact Go variant of the TigerStyle Strict rulebook. Each rule is an imperative with a
one-line rationale. For full rationale and Go examples, see `tigerstyle-go-strict-full.md`.
...
**Go tooling:** `gofmt` (tabs) REQUIRED. `go vet` and `staticcheck` MUST pass. `go test ./...` MUST
pass; use `-race` in CI for concurrency-heavy code.
...
**SAF-03** **(Go)** — Integer types MUST be explicitly sized (int32, uint64). `int`/`uint` MUST NOT be used except where stdlib requires.
...
**SAF-12** **(Go)** — Allocations in hot paths MUST be minimized. Pre-allocate slices/buffers and reuse.
...
**SAF-17** **(Go)** — `gofmt`, `go vet`, and `staticcheck` MUST pass. Warnings are errors.
...
**PERF-08** **(Go)** — Hot loop functions MUST take primitive arguments. Avoid receiver access in tight loops.
...
**DX-04** — Acronyms MUST use standard capitalization (HTTPServer, SQLClient).
...
**DX-06** — Resource names MUST convey lifecycle and ownership (pool, arena, buffer).
...
**DX-10** — Important declarations (exported API) MUST appear first in a file.
...
**CIS-02** **(Go)** — Function arguments larger than 16 bytes MUST be passed by pointer.
Rationale: Avoids implicit copies and stack waste.

**CIS-03** **(Go)** — Large structs MUST be initialized in-place via pointers/builders, not returned by value.
Rationale: Avoids copies and ensures pointer stability.

**CIS-04** **(Go)** — If any field requires in-place init, the entire struct MUST be initialized in-place.
...
**CIS-06** — Return types MUST be as simple as possible: void > bool > int > *T > Result-like.
Rationale: Each dimension in the return type creates viral call-site branching.

**CIS-07** **(Go)** — Assertions MUST remain valid across goroutines; avoid goroutine boundaries between assert and use.
Rationale: Concurrency can invalidate preconditions and mislead assertions.
...
**FMT-01** — All code MUST be formatted by `gofmt`.
Rationale: Eliminates style debates and ensures consistency.

**FMT-02** **(Go)** — Indentation MUST use gofmt tabs. No spaces for indentation.
...
**DEP-03** **(Go)** — Scripts MUST be written in Go. Shell scripts only for trivial glue (<20 lines).
