# TigerStyle Rulebook — Go / Pragmatic / Full
...
### Go Tooling Recommendations

- `gofmt` SHOULD be run on all Go files (tabs are the default).
- `go vet` SHOULD pass with no warnings.
- `staticcheck` SHOULD pass with no warnings (or an equivalent lint toolchain).
- `go test ./...` SHOULD pass; `-race` SHOULD be used in CI for concurrency-heavy code.
...
- Each rule has: a recommendation, a rationale, and a Go example or template.
...
Integer types SHOULD be explicitly sized (`int32`, `uint64`). AVOID `int`/`uint` unless required by
the Go standard library (e.g., `len`, indices).
...
Assertions SHOULD cover both the positive space (expected) and negative space (not expected).
...
Rationale: Most catastrophic failures stem from incorrect error handling.

### SAF-12 — Pre-allocate and reuse. Avoid unbounded allocations. **(Go-adapted)**

Allocations in hot paths SHOULD be minimized. Pre-allocate slices/buffers where possible. Reuse
buffers across calls. CONSIDER `sync.Pool` when it demonstrably helps.
...
### SAF-17 — Treat warnings as errors; prefer vet and staticcheck. **(Go-adapted)**

`gofmt`, `go vet`, and `staticcheck` SHOULD pass with no warnings.
...
Every error SHOULD be handled explicitly. AVOID ignoring errors with `_`.

Rationale: Error-handling bugs are the dominant cause of catastrophic failures.
...
### PERF-08 — Use primitive arguments in hot loops. Avoid receiver access. **(Go-adapted)**

Hot loop functions SHOULD take primitive arguments directly. AVOID accessing large receivers in
tight loops.
...
### DX-02 — Use Go naming idioms. **(Go-adapted)**

Use `mixedCaps` for identifiers, `MixedCaps` for exported names, and `snake_case` for file names.

Rationale: Consistency with Go conventions reduces friction.

### DX-03 — Do not abbreviate names (except trivial loop counters).

Names SHOULD NOT be abbreviated unless the variable is a trivial loop counter (`i`, `j`, `k`).

Rationale: Abbreviations are ambiguous.

### DX-04 — Capitalize acronyms consistently.

Acronyms SHOULD use standard capitalization (HTTPServer, SQLClient).
...
Nullable parameters SHOULD be named so `nil` meaning is clear.
...
Arguments larger than 16 bytes SHOULD be passed by pointer.
...
Return types SHOULD be as simple as possible: void > bool > int > *T > Result-like.
...
Avoid goroutine boundaries between assertions and dependent code. Re-assert after synchronization.
...
All code SHOULD be formatted by `gofmt`.

Rationale: Eliminates style debates and ensures consistency.

### FMT-02 — Indentation SHOULD follow gofmt (tabs). **(Go-adapted)**

Indentation SHOULD use tabs as produced by `gofmt`.
...
Scripts SHOULD be written in Go. Shell scripts are acceptable only for trivial glue (< 20 lines).
...
| FMT-01 | Run gofmt | |
| FMT-02 | gofmt tabs preferred | Yes |
