...
a one-line rationale. For full rationale and C examples, see `tigerstyle-c-strict-full.md`.
...
**C baseline:** C17. `clang-format` required. `clang` and `gcc` warnings as errors. Sanitizers in
CI.
...
**SAF-03** **(C)** — Integer types MUST be explicitly sized (`uint32_t`, `int64_t`). `size_t` MUST
NOT be used except for indexing/API needs.
...
**CIS-02** **(C)** — Function arguments larger than 16 bytes MUST be passed by `const` pointer.
