...
with a one-line rationale. For full rationale and C examples, see `tigerstyle-c-pragmatic-full.md`.
...
**C baseline:** C17. `clang-format` recommended. `clang`/`gcc` warnings as errors. Sanitizers in CI.
...
**SAF-03** **(C)** — Integer types SHOULD be explicitly sized (`uint32_t`, `int64_t`). AVOID
`size_t` except for indexing/API needs.
...
**DX-02** **(C)** — File, function, and variable names SHOULD use snake_case; types follow project
convention.
...
**CIS-02** **(C)** — Function arguments larger than 16 bytes SHOULD be passed by const pointer.
...
**CIS-06** — PREFER simpler return types: void > bool > int > optional > error struct.
