...
- Type-punning MUST use `memcpy`, not aliasing casts.
...
Integer types MUST be explicitly sized (`uint32_t`, `int64_t`). `size_t` MUST NOT be used unless
...
uint32_t count = 0;
int64_t offset = 0;
size_t length = buffer_length; /* Only for API size/index. */
```
...
```c
void transfer(struct account *from, struct account *to, int64_t amount) {
    assert(from != NULL);
    assert(to != NULL);
    assert(from != to);
    assert(amount > 0);
...
```c
uint32_t process_batch(const struct batch *batch, uint32_t max_size) {
    assert(batch != NULL);
...
asserted at compile time (`_Static_assert`) or at program startup.

Rationale: Compile-time assertions verify design integrity before the program executes.

```c
_Static_assert(sizeof(struct header) == 64, "header size must be 64 bytes");
_Static_assert(MAX_BATCH_SIZE <= BUFFER_CAPACITY, "batch too large");
```
...
initialize(buffer, BUFFER_CAPACITY);
```
...
for (uint32_t index = 0; index < count; index++) {
    uint32_t checksum = compute_checksum(items[index]);
...
```c
void update_balance(struct account *account, int64_t amount) {
    int64_t new_balance = compute_new_balance(account->balance, amount);
    assert(new_balance >= 0);
    account->balance = new_balance;
}

int64_t compute_new_balance(int64_t balance, int64_t amount) {
...
if (queue_count(&event_queue) >= MAX_BATCH) {
...
write_all(batch.items, batch.count);
```
...
```c
void hot_loop(const uint8_t *data, uint32_t length, uint32_t stride) {
...
```c
typedef void (*read_callback)(int status);
void read_sector(int fd, uint32_t sector_id, read_callback callback);
```
...
void transfer(struct transfer_options options);

transfer((struct transfer_options){ .from = a, .to = b, .amount = 100 });
```
...
struct connect_options {
    const char *host;
    const uint32_t *timeout_ms_or_null;
};
```
...
Function arguments larger than 16 bytes MUST be passed by `const` pointer, not by value.

Rationale: Passing large structs by value creates implicit copies.

```c
void process_config(const struct config *config);
```
...
```c
void init_config(struct config *out_config);
```
...
```c
void init_container(struct container *out_container) {
...
```c
uint32_t offset = compute_offset(index);
buffer[offset] = value;
```

### CIS-06 — Prefer simpler return types to reduce call-site dimensionality.

Return types MUST be as simple as possible. PREFER `void` over `bool`, `bool` over `int`, `int` over
optional, optional over error structs.

Rationale: Each additional dimension creates branches at every call site.

```c
void validate_input(const struct input *input) {
...
```c
memset(buffer + data_length, 0, BUFFER_SIZE - data_length);
```
...
```c
uint32_t last_index = 9;
uint32_t count = last_index + 1;
uint32_t size_bytes = count * ITEM_SIZE;
```
...
```c
uint32_t pages = div_ceil(total_bytes, PAGE_SIZE);
uint32_t slots = div_exact(buffer_size, SLOT_SIZE);
```
...
```text
Prefer: scripts/deploy.c, scripts/migrate.py
