# TigerStyle Rulebook — Zig / Pragmatic / Full
...
- Runtime safety SHOULD NOT be disabled except in audited hot paths with documented invariants.
...
- Each rule has: a recommendation, a rationale, and a Zig example or template.
...
Rationale: Implicit sizing creates architecture-specific behavior.
...
`zig fmt` and `zig build test` SHOULD pass with no warnings. Avoid disabling runtime safety except
in audited hot paths.
...
Every error SHOULD be handled explicitly. Avoid silently ignoring errors.
...
File, function, and variable names SHOULD use `snake_case`. Use Zig naming conventions for types.
...
Struct definitions SHOULD be ordered: fields first, then nested types, then methods.
...
Arguments larger than 16 bytes SHOULD be passed by `*const`, not by value.
...
PREFER simpler return types: void > bool > integer > optional > error union.

Rationale: Each dimension creates viral call-site branching.

### CIS-07 — Do not suspend between assertions and dependent code.

AVOID suspending between an assertion and the code that depends on it.
...
All integer division SHOULD use explicit semantics: `@divExact`, `@divFloor`, or `@divCeil`.
...
All code SHOULD be formatted by `zig fmt`.
...
Scripts SHOULD be written in Zig. Shell scripts only for trivial glue (< 20 lines).
