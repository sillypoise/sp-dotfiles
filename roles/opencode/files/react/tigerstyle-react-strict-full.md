# TigerStyle Rulebook — React (TypeScript) / Strict / Full
...
TigerStyle for cross-referencing. This is the strict variant: MUST/SHALL rules are requirements.
...
Non-compliance with any MUST/SHALL rule is a blocking review finding unless the rule is
explicitly marked as not applicable to the project in a project-level override document.
...
- Each rule has: an imperative statement, a rationale, and a React/TypeScript example or template.
...
All loops, queues, retries, buffers, and backoffs MUST have fixed upper bounds.
...
Numeric domains (ids, counts, indexes) MUST be explicit and not rely on implicit coercion.
`any` and unsafe casts MUST NOT be used.
...
Functions MUST assert their preconditions, postconditions, and invariants.

Rationale: UI bugs are easier to detect when invalid states fail fast.

React note: render MUST be pure. Browser APIs and side effects MUST NOT run in render.
...
The codebase MUST average at least two assertions per function.
...
Every enforced property MUST have at least two assertions on different code paths.

Rationale: Bugs hide at boundary transitions.

### SAF-07 — Split compound assertions.

Compound assertions MUST be split into independent checks.
...
Implication checks MUST be expressed as `if (a) assert(b)`.
...
Compile-time constants and environment requirements MUST be asserted at startup.
...
Assertions MUST cover expected and not-expected ranges.

Rationale: Boundary bugs are common in UI state.

### SAF-11 — Test valid, invalid, and boundary transitions.

Tests MUST cover valid inputs, invalid inputs, and boundary transitions.

Rationale: Most failures are invalid state transitions.

### SAF-12 — Avoid unbounded allocations in render. **(React-adapted)**

Static allocations and reuse MUST be used in hot paths. Per-render allocations MUST be avoided
when costly.
...
All compiler/linter warnings MUST be enabled and resolved. TypeScript `strict` and `oxc`
...
External events MUST be batched and processed in controlled updates.

Rationale: Batching prevents render storms.

React note: user interactions MUST be handled in event handlers. Effects are only for external
synchronization. Non-urgent updates MUST use transitions.
...
Compound boolean conditions MUST be split into nested branches.

Rationale: Explicit branches are easier to verify.

### SAF-20 — State invariants positively.

Invariants MUST be stated in positive form. Negations MUST NOT be used.

Rationale: Positive statements reduce logical errors.

### SAF-21 — Handle all errors explicitly. **(React-adapted)**

Errors MUST be handled explicitly. Ignored promises and silent catches MUST NOT be used.

Rationale: Silent failures mask UI corruption.

React note: Error Boundaries MUST be used for render-time failures; async rejections handled.
...
Non-obvious decisions MUST have a “why” comment or commit message.

Rationale: Maintainers need the rationale, not the restatement.

### SAF-23 — Pass explicit options. Avoid defaults.

All options MUST be passed explicitly to library calls.
...
Performance-critical code MUST be explicit. Compiler heuristics MUST NOT be relied on.

Rationale: Compiler heuristics are fragile.

### PERF-08 — Use primitive arguments in hot loops. **(React-adapted)**

Hot path components MUST receive primitives or stable references.

Rationale: Stable props reduce re-renders and memo misses.

React note: inline objects/functions in list items MUST be avoided; use stable callbacks.
...
Functions/variables MUST use lowerCamelCase; components use UpperCamelCase.

Rationale: Consistency improves tooling and readability.

### DX-03 — Do not abbreviate names.

Names MUST NOT be abbreviated except trivial loop counters.
...
Units and qualifiers MUST be appended to names, sorted by significance.
...
Nullable parameters MUST make `null`/`undefined` meaning clear.

Rationale: `foo(undefined)` is meaningless without context.

### DX-16 — Singleton params ordered general → specific.

Singleton constructor params MUST be ordered from general to specific.
...
Tests MUST describe goal and methodology.
...
Every piece of state MUST have exactly one source of truth.

Rationale: Duplicate state desynchronizes.

React note: derived state MUST stay derived. Use fallback patterns (`value ?? prop`) and
...
Large objects MUST NOT be passed as props when an id or primitive suffices.

Rationale: Large props cause re-render churn.

### CIS-03 — Prefer lazy initialization for expensive state. **(React-adapted)**

Expensive initial state MUST be computed lazily.
...
If a value can be derived, the entire derived chain MUST be derived at render time.

Rationale: Mixed storage causes drift.

React note: syncing state in effects MUST be avoided; use keys to reset state or derive.

### CIS-05 — Declare variables close to use.

Variables MUST be declared and computed near use.

Rationale: Reduces TOCTOU errors.

### CIS-06 — Prefer simpler return types.

Return types MUST prefer: void > bool > number > optional > Result.
...
If using typed arrays or binary buffers, unused space MUST be zeroed.

Rationale: Buffer underflow can leak data.

### CIS-09 — Group setup and cleanup in effects. **(React-adapted)**

Setup and cleanup MUST be visually grouped in a single effect.

Rationale: Reduces resource leaks in subscriptions.

React note: `useSyncExternalStore` MUST be preferred over manual subscriptions when possible.
...
Indexes, counts, and sizes MUST be treated as distinct concepts.

Rationale: Off-by-one errors dominate list bugs.

### OBO-02 — Use explicit division semantics.

Division semantics MUST be explicit (exact, floor, ceiling).
...
All code MUST be formatted by the project formatter.

Rationale: Automated formatting reduces review noise.

### FMT-02 — Use the project’s indentation standard.

Indentation MUST follow the project standard (default 2 spaces in JS/TS).
...
If statements MUST have braces unless single-line.
...
New tools MUST NOT be introduced when existing tools suffice.

Rationale: Tool sprawl increases maintenance cost.

### DEP-03 — Prefer typed tooling for scripts. **(React-adapted)**

Scripts MUST be written in typed, portable languages (TypeScript, Node). Shell is allowed only
for trivial glue.
