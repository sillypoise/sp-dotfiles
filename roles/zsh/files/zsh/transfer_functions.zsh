...
_transfer_peer_resolves() {
  ...
  getent hosts "$peer_host" >/dev/null 2>&1
}
...
_transfer_connection_hint() {
  ...
  if ! _transfer_peer_resolves "$peer_host"; then
    print "  hint: host does not resolve locally: $peer_host"
    print "  hint: check tailscale dns settings or use an IP as host"
  fi
...
_transfer_run() {
  ...
  if ! _transfer_peer_resolves "$peer_host"; then
    print "transfer: peer host does not resolve locally: $peer_host"
    print "  hint: verify tailscale dns (accept-dns/magicdns) or use user@ip"
    return 1
  fi
...
rctx() {
  local short_host fqdn_host peer_host peer_state ssh_client

  short_host="$(hostname -s 2>/dev/null || true)"
  fqdn_host="$(hostname -f 2>/dev/null || true)"

  print "host: ${short_host:-unknown}"
  if [[ -n "$fqdn_host" && "$fqdn_host" != "$short_host" ]]; then
    print "fqdn: $fqdn_host"
  fi
  ...
  if [[ -z "$RDS_PEER" ]]; then
    ...
    print "peer: $RDS_PEER ($peer_state)"
  fi

  if [[ -n "${SSH_CONNECTION:-}" ]]; then
    ssh_client="${SSH_CONNECTION%% *}"
    print "via ssh: $ssh_client"
  else
    print "via ssh: no"
  fi
}
